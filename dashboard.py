...
from database import get_all_students, get_student_by_id, get_student_quiz_history
...
st.set_page_config(
    page_title="Student Dashboard - PathAI", page_icon="📈", layout="wide"
)
...
def generate_pdf_report(student, quiz_history_df):
    ...
    story.append(
        Paragraph("Personalized Learning Performance Report", title_style)
    )
    story.append(
        Paragraph(
            f"Student Name: <b>{student['name']}</b> (ID: {student['student_id']})",
            styles["Normal"],
        )
    )
    story.append(
        Paragraph(
            f"Education Level: {student['education_level']} | Learning Style:"
            f" {student['learning_style']}",
            styles["Normal"],
        )
    )
    ...
    story.append(
        Paragraph("<b>Topic Mastery Breakdown:</b>", styles["Heading2"])
    )
    story.append(
        Paragraph(
            f"• <b>Strong Topics:</b> {student['strong_topics']}",
            styles["Normal"],
        )
    )
    story.append(
        Paragraph(
            f"• <b>Weak Topics Needing Focus:</b> {student['weak_topics']}",
            styles["Normal"],
        )
    )
    story.append(Spacer(1, 16))
    ...
    if not quiz_history_df.empty:
        story.append(
            Paragraph("<b>Recent Quiz History:</b>", styles["Heading2"])
        )
        qh_data = [["Topic", "Score", "Attempts", "Time Spent (m)"]]
        for _, row in quiz_history_df.head(5).iterrows():
            qh_data.append([
                str(row.get("topic_title", "Quiz Module")),
                f"{row.get('score', 0)}%",
                str(row.get("attempts", 1)),
                f"{row.get('time_spent', 0)} mins",
            ])
...
if not st.session_state.get("authenticated"):
    st.warning(
        "Please log in from the main app page to view your Student Dashboard."
    )
    st.stop()
...
current_student_id = st.session_state.get("student_id", "STU0001")
...
with col_sel1:
    if role == "student":
        ...
        if isinstance(students_data, pd.DataFrame):
            students_df = students_data
        else:
            students_df = pd.DataFrame(students_data)

        if not students_df.empty:
            # Map display text "Name (ID)" -> student_id to resolve duplicate names
            student_map = {
            ...
            selected_id = student_map[selected_option]
            student = get_student_by_id(selected_id)
        else:
            student = None
            st.error("No students found in database.")

# Ensure student record is a dictionary format
if isinstance(student, pd.DataFrame):
    student = student.iloc[0].to_dict() if not student.empty else None
    ...
    <div class="glass-card">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div>
                ...
                <p style="margin:0; color:#A0AEC0;">ID: {student['student_id']} | Email: {student.get('email', 'N/A')}</p>
            </div>
            <div>
...
with m1:
    st.metric("Quiz Score Accuracy", f"{student['quiz_score']}%")
with m2:
    st.metric("Course Progress", f"{student['progress']}%")
with m3:
    st.metric("Time Spent per Module", f"{student['time_spent']} mins")
with m4:
    st.metric("Quiz Attempts", f"{student['attempts']}")
...
with col_left:
    ...
    if not quiz_history_df.empty:
        ...
        sim_data = pd.DataFrame({
            "Attempt": [f"Quiz {i}" for i in range(1, 6)],
            "Score": [
                max(40, student["quiz_score"] - 20 + i * 5) for i in range(5)
            ],
        })
        fig_trend = px.bar(
            sim_data,
...
with col_right:
    ...
    st.markdown(
        f"""<div class="glass-card" style="border-left: 4px solid #10B981;"><h4 style="color:#10B981; margin:0 0 8px 0;">💪 Strong Topics</h4><p style="margin:0;">{student['strong_topics']}</p></div>""",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"""<div class="glass-card" style="border-left: 4px solid #EF4444;"><h4 style="color:#EF4444; margin:0 0 8px 0;">⚠️ Weak Topics Needing Revision</h4><p style="margin:0;">{student['weak_topics']}</p></div>""",
        unsafe_allow_html=True,
    )
